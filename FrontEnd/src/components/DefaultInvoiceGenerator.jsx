import {useLocation as location, useNavigate } from "react-router-dom";
...
function InvoiceGenerator(props) {
  ...
  const handleConfirmOrder = async () => {
    try {
      ...
      const myobj = {
        userId: userid,
        modelId: props.seelctedId,
        orderedQty: props.orderSize,
        altCompId: [],
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:8080/api/models/${props.seelctedId}`)
  ...
  useEffect(() => {
    ...
    if (modelPrice !== null && props.orderSize !== null) {
      const basePrice = modelPrice * props.orderSize; // Calculate base price by multiplying model price with order size
  ...
  const generateAndDownloadPDF = () => {
    ...
    // Order Size
    doc.text(`Order Size: ${props.orderSize}`, 10, 50);
